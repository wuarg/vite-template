<script setup>
  ...
  function loadMapTokens() {
    axios
      .get(
        'https://loopworms.io/DEV/LooperLands/Maps/map_picker_degroot.php?WalletID=' +
      ...
      .then((response) => {
        ...
        if (tokens.value.length <= 0 && !notified.value) {
          ...
          //   description:
          //     'No tokens found for this wallet. Please make sure you are connected to the correct wallet.',
          //   duration: 5000,
          // });
        }
      });
...
<template>
  <n-button v-if="!account.isConnected" class="text-current" type="info" @click="openModal"
    >Connect wallet</n-button
  >

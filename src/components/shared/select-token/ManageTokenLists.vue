<template>
  ...
  <template v-if="activeTab === 'lists-tab'">
    <div class="search-input-holder">
      <Input v-model="searchText" size="large" placeholder="Search name or paste address" />
    </div>
    <ul class="token-providers-list">
      <li v-for="provider in tokensProviders" :key="provider.name.replace(/ /g, '-')" class="item">
        <img class="item-image" :src="provider.logoURI" :alt="provider.name" />
        <div class="item-name-holder">
          <h5 class="item-name">{{ provider.name }}</h5>
          <p v-if="provider.tokens.length" class="tokens-count">
        ...
        <SwitchInput
          ...
          @update:modelValue="updateTokensProvider({ ...provider, enabled: $event })"
        />
      </li>
    </ul>
  </template>
  <template v-if="activeTab === 'tokens-tab'">
    <div class="search-input-holder">
      <BaseInput v-model="customTokenAddress" size="large" placeholder="0x0000" />
    </div>
    <div class="custom-tokens">
      <p class="note"> 0 Custom Tokens </p>
    </div>
    <p class="tip"> Tip: Custom tokens are stored locally in your browser </p>
  </template>
</template>
